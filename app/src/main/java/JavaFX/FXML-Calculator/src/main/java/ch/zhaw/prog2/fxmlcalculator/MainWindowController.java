...
public class MainWindowController implements ConnectController {
	...
	public void setModel(ValueHandler model) {
		valueHandler = model;
        // bind the result string of the valueHandler (model) to the result TextField-property
	...
	private void calculateResultBound() {
		valueHandler.checkAndCalculateResult(initialAmount.getText(), returnInPercent.getText(), annualCost.getText(), numberOfYears.getText());
	...
	private void openResultWindow() {
		try {
			...
			controller.setModel(valueHandler);
