...
public class ResultWindowController implements ConnectController {
	...
	@Override
	/**
	 * Is called from Main to connect the Controller to the model
	 ...
	 */
	public void setModel(ValueHandler model) {
		this.valueHandler = model;
		// either bind the result string of the valueHandler (model) to the results TextField-property
		// results.textProperty().bind(valueHandler.resultBoundProperty());
		// or register a ChangeListener and also set the border color based on the state from of the value handler
        ...
        // set the result text field with current value and status from the model
        updateResult();
	}
